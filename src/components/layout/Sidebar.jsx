        ...
        {navItems.map((item) => {
          ...
          return (
            <Link key={item.path} to={item.path}>
              ...
              >
                <item.icon className={`w-4 h-4 flex-shrink-0 ${isActive ? 'text-cyan-400' : 'text-slate-500 group-hover:text-slate-300'}`} />
                <span className="font-body font-500">{item.label}</span>
      ...
      <div className="p-3 border-t border-white/[0.06] space-y-1">
        ...
        <div className="flex items-center gap-3 px-3 py-2.5 mt-2 bg-white/[0.03] rounded-xl border border-white/[0.05]">
          <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-purple-500 to-cyan-500 flex items-center justify-center flex-shrink-0">
            <span className="text-xs font-display font-700 text-white">{initials}</span>
          </div>
